...
formatter = logging.Formatter('%(asctime)s %(name)s %(lineno)-10s %(levelname)s %(message)s')#'%(asctime)s %(name) - %(lineno)-10s %(levelname)-8s %(message)s')
...
.../LDST/
    ...
    │       └── ...
    └── plots
            ├── YYYYMMDD.png    #Plots of the nights schedule
            └── ...
"""